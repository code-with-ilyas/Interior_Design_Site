                    ...
                    <form action="{{ route('admin.gallery.update', $gallery->id) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="md:col-span-2">
                                ...
                                @if($gallery->image)
                                    <div class="mb-2">
                                        <img src="{{ asset($gallery->image) }}" alt="{{ $gallery->title }}" class="w-32 h-32 object-cover rounded">
                                    </div>
                                @endif
                                <input type="file" name="image" id="image" accept="image/*"
                                       class="mt-1 block w-full rounded-md border-gray-300 shadow-sm 
                                              focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                <p class="mt-1 text-sm text-gray-500">Upload a new image (JPG, PNG, GIF) - Max 2MB</p>
                            </div>
                        </div>
                        ...
                        <div class="mt-6 flex items-center justify-end">
                            <a href="{{ route('admin.gallery.index') }}" 
                               class="mr-3 px-4 py-2 bg-gray-500 text-dark rounded hover:bg-gray-600">
                                Cancel
                            </a>
                            <button type="submit" 
